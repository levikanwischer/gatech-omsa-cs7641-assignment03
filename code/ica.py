...
"""This module contains general code for ICA."""
...
from sklearn.decomposition import FastICA as DimRedux
...
def search_best_k(datasets, targets):
    ...
    for dataset, target in zip(datasets, targets):
        ...
        logging.info(f"Initializing ICA search for {dataset}...")
        ...
        for k in range(1, train.X.shape[1]):
            dim = DimRedux(k, max_iter=5000, whiten=True, random_state=42)
            dim.fit(train.X)
        ...
        outpath = os.path.join(helpers.BASEDIR, "img", f"dim-ica-both.png")
        plt.savefig(outpath)
...
def append_cluster_labels(train, test, bestK):
    ...
    clu = DimRedux(bestK, max_iter=5000, whiten=True, random_state=42)
    clu.fit(train.X)
...
if __name__ == "__main__":
    ...
    plot_component_kurtosis(datasets, targets)
