...
"""This module contains general code for Expectation Maximization."""
...
from sklearn.mixture import GaussianMixture as Clusterizer
...
def search_best_k(datasets, targets, kMax=20, kMin=2, extra=""):
    """Search for best K by BIC & Silhouette."""

    for dataset, target in zip(datasets, targets):

        logging.info(f"Initializing EM search for {dataset}...")
...
def _sub_search_best_k(dataset, train, test, kMax, kMin, extra=""):
    ...
    bic, sil, nmi = dict(), dict(), dict()
    for k in range(kMin, kMax + 1):
        clu = Clusterizer(k, max_iter=1000, random_state=42)
        clu.fit(train.X)

        bic[k] = clu.bic(train.X)
        logging.info(f"Bayesian Info Criterion for {k} clusters is {bic[k]:.4f}")
    ...
    plt.title(f"{dataset} - BIC", fontsize=10)
    ...
    outpath = os.path.join(helpers.BASEDIR, "img", f"em-bestk-{extra}{dataset}.png")
    plt.savefig(outpath)
    return None
